...
-- Hàm trích xuất URL từ game:HttpGet("URL") hoặc game:HttpGet("URL", true)
local function extractUrl(content)
    local url = content:match('game:HttpGet%s*%(%s*"([^"]+)"%s*,?%s*true?%s*%)')
    if not url then -- Nếu không tìm thấy, thử lại với pattern không có `, true`
        url = content:match('game:HttpGet%s*%(%s*"([^"]+)"%s*%)')
    end
    return url
end
	local runcode = runcode and clonefunction(runcode) or function(scr)
		local content = scr
		local script
    if content:find('test%(%s*"request"%s*,%s*{"http%.request"%s*,%s*"http_request"%s*},%s*function%(') then
        script = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/haho777/sad1/refs/heads/main/un"))()'
        loadstring(script)()
    else
		if content:find('acsu123') then
			loadstring(content)()
		else
        local url = extractUrl(content)
        if url then
                local success, response = pcall(function()
                    return game:HttpGet(url)
                end)
                if success and response then
                    if response:find('test%(%s*"request"%s*,%s*{"http%.request"%s*,%s*"http_request"%s*},%s*function%(') then
                        script = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/haho777/sad1/refs/heads/main/un"))()'
                    else
                        script = luaCode .. content
                    end
                else
                    print("Script không tồn tại hoặc lỗi khi tải mã từ URL")
                    return
                end
            loadstring(script)()
        else
            loadstring(luaCode .. content)()
        end
	end
    end
	end
	...
	local function createUI(directory: Instance): ScreenGui
		ui = instanceUtils:Create("ScreenGui", {
			...
			textLabel({
				...
				Text = "Please complete the whitelist to gain access to TNG",
				TextColor3 = Color3.fromRGB(159, 164, 186)
			}),
			instanceUtils:Create("Frame", {
				AnchorPoint = Vector2.new(1, 0),
